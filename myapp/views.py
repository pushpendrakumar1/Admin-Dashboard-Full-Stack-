...
def set_session(request):
    request.session['user_id'] = 123  # Store user-specific data in the session
    return HttpResponse("Session data set.")

def get_session(request):
    user_id = request.session.get('user_id', None)  # Retrieve data from the session
...
def process_form(request):
    if request.method == 'POST':
        ...
        input_file = fs.save(uploaded_filename, uploaded_file)
        ...
        # Create a new instance of LogHistory
        log_entry = log_history()
        log_entry.uploaded_file = fs.save(uploaded_filename, uploaded_file)
        log_entry.processed_file = fs.save(output_file, open(output_path, 'rb'))
        log_entry.output_file = log_entry.processed_file
        log_entry.save()
...
# -------------------------edp-history------------------------
def Edp_history(request):
    xlsx_files = log_history.objects.all().order_by('-date_modified')

    context = {'xlsx_files': xlsx_files}
    return render(request, 'Edp_history.html', context)
